...
func main() {
	...
	var arr10 = [10]int{1,2,3}//没达到指定长度 0补齐
	fmt.Println("arr10",arr10)
	// 初始化一个字符串数组，数组长度为2
	var arr2 = [2]string{"hello", "world"}
	var arrKeyValue = [5]string{3: "Chris", 4: "Ron"}//这种形式 key必须是整形 也就是数组索引
	for h:=0; h < len(arrKeyValue); h++ {
	...
	//数组混合类型
	mixed := [3]interface{}{"foo", 10, []int{1,2}}
	fmt.Println(mixed)
