...
func main() {
	...
	//变量名称区分大小写
	var v0 int
	_ = v0 //占位符，意思是那个位置本应赋给某个值，意思是丢掉不要。释放内存？
	...
	//变量交换
	v13, v14 := 10, 20//必须是同一种类型才行

	var h int = 3 //定义了必须使用 否则报错
	fmt.Printf("%d",h)//println只能接收字符串 打印其他类型的必须格式化 否则报错
	//var vv string = "vv"
	//h,vv = vv,h//报错 因为v h不是同一个类型
